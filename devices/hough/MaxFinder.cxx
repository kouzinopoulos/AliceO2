...
MaxFinder::MaxFinder(Char_t* histotype, Int_t nmax, Accumulator* hist)
{
  // Constructor

  // fTracks = new AliHLTTPCTrackArray("HoughTrack");
  if (strcmp(histotype, "KappaPhi") == 0)
    fHistoType = 'c';
  if (strcmp(histotype, "DPsi") == 0)
    fHistoType = 'l';
...
void MaxFinder::WriteNtuppel(Char_t* filename)
{
  // Write the NTuple with the peak parameters
  TFile* file = TFile::Open(filename, "RECREATE");
  if (!file) {
    cerr << "MaxFinder::WriteNtuppel : Error opening file " << filename << endl;
    return;
  }
  fNtuppel->Write();
  file->Close();
...
void MaxFinder::FindAbsMaxima()
{
  // Simple Peak Finder in the Hough space
  if (!fCurrentHisto) {
    cerr << "MaxFinder::FindAbsMaxima : No histogram" << endl;
    return;
  }
  Accumulator* hist = fCurrentHisto;
  ...
  if (fNPeaks > fNMax) {
    cerr << "MaxFinder::FindAbsMaxima : Array out of range : " << fNPeaks << endl;
    return;
  }
  ...
  if (fNtuppel) {
    ...
    fNtuppel->Fill(maxx, maxy, maxvalue, hist->GetBinContent(bin3), hist->GetBinContent(bin5),
                   hist->GetBinContent(bin1), hist->GetBinContent(bin7));
  }
}

void MaxFinder::FindBigMaxima()
{
  // Another Peak finder
  Accumulator* hist = fCurrentHisto;
  ...
  for (Int_t xbin = xmin + 2; xbin < xmax - 3; xbin++) {
    for (Int_t ybin = ymin + 2; ybin < ymax - 3; ybin++) {
      ...
      Int_t b = 0;
      while (1) {
        if (value[b] > value[12] || b == binindex)
          break;
        b++;
        // printf("b %d\n",b);
      }
      if (b == binindex) {
        // Found maxima
        if (fNPeaks > fNMax) {
          cerr << "MaxFinder::FindBigMaxima : Array out of range " << fNPeaks << endl;
          return;
        }
...
void MaxFinder::FindMaxima(Int_t threshold)
{
  ...
  for (Int_t xbin = xmin + 1; xbin <= xmax - 1; xbin++) {
    for (Int_t ybin = ymin + 1; ybin <= ymax - 1; ybin++) {
      ...
      if (value[4] > value[0] && value[4] > value[1] && value[4] > value[2] && value[4] > value[3] &&
          value[4] > value[5] && value[4] > value[6] && value[4] > value[7] && value[4] > value[8]) {
        // Found a local maxima
        Float_t maxx = fCurrentHisto->GetBinCenterX(xbin);
        Float_t maxy = fCurrentHisto->GetBinCenterY(ybin);

        if ((Int_t)value[4] <= threshold)
          continue; // central bin below threshold
        if (fNPeaks >= fNMax) {
          cout << "MaxFinder::FindMaxima : Array out of range " << fNPeaks << endl;
          return;
        }
...
void MaxFinder::FindAdaptedPeaks(Int_t kappawindow, Float_t cutratio)
{
  ...
  if (!hist) {
    cerr << "MaxFinder : No histogram!" << endl;
    return;
  }
    ...
    for (Int_t i = 0; i < nmaxs[ybin - ymin]; i++) {
      ...
      while (yl >= ymin) {
        ...
        for (Int_t j = 0; j < nmaxs[yl - ymin]; j++) {
          if (localmaxima[yl - ymin][j].fStart - lb < 0)
            continue;
          if (localmaxima[yl - ymin][j].fStart < lb + kappawindow + match && localmaxima[yl - ymin][j].fStart >= lb &&
              localmaxima[yl - ymin][j].fSum > 0) {

            // cout<<"match at ybin "<<yl<<" yvalue "<<hist->GetBinCenterY(yl)<<" start
            // "<<localmaxima[yl-ymin][j].start<<" sum "<<localmaxima[yl-ymin][j].sum<<endl;
        ...
        if (!found ||
            yl == ymin) // no more local maximas to be matched, so write the final peak and break the expansion:
        {
          if (nybins > 4) {
...
void MaxFinder::FindAdaptedRowPeaks(Int_t kappawindow, Int_t xsize, Int_t ysize)
{
  // Peak finder which is working over the Hough Space provided by the TransformerRow class
  Accumulator* hist = fCurrentHisto;
  ...
  for (Int_t ybin = fNextRow[ymin]; ybin <= ymax; ybin = fNextRow[ybin + 1]) {
    ...
    Bool_t found = 0;
    for (Int_t xbin = xmin; xbin <= xmax; xbin++) {
      value = content[xbin + nxbins * ybin]; // value = hist->GetBinContent(xbin + nxbins*ybin); //value =
                                             // hist->GetBinContent(hist->GetBin(xbin,ybin));
      if (value > 0) {
    ...
    for (Int_t i = 0; i < nmaxs[ybin - ymin]; i++) {
      ...
      while (localy >= ymin) {
        ...
        for (Int_t j = 0; j < nmaxs[localy - ymin]; j++) {
          if ((localmaxima[localy - ymin][j].fStartPosition <= (tempxend + kappawindow)) &&
              (localmaxima[localy - ymin][j].fEndPosition >= (tempxstart - kappawindow))) {
            if ((localmaxima[localy - ymin][j].fMinValue <= localmaxvalue) &&
                (localmaxima[localy - ymin][j].fMaxValue >= localminvalue)) {
        ...
        if (!found ||
            localy == ymin) // no more local maximas to be matched, so write the final peak and break the expansion:
        {
          if ((nybins > ysize) && ((localxend - localxstart + 1) > xsize) && (localmaxvalue > localprevvalue) &&
              (localmaxvalue > localnextvalue) && (localmaxvalue > localleftvalue) && (localmaxvalue > localrightvalue))
...
#ifdef do_mc
//		      cout<<"Peak found at: "<<((Float_t)localxstart+(Float_t)localxend)/2.0<<"
//"<<((Float_t)ybin+(Float_t)(localy+1))/2.0<<" "<<localminvalue<<" "<<localmaxvalue<<" "<<" with weight
//"<<(localminvalue+localmaxvalue)/2<<" and size "<<localxend-localxstart+1<<" by "<<nybins<<endl;
#endif
            nmaxima++;
          }
          break;
  ...
  for (Int_t i = 0; i < nmaxima; i++) {
    ...
    for (Int_t j = fN1PeaksPrevEtaSlice; j < fN2PeaksPrevEtaSlice; j++) {
      ...
      if ((fENDETAPeaks[j] - fSTARTETAPeaks[j]) >= 2)
        continue;
      if ((maxima[i].fStartX <= fENDXPeaks[j] + 1) && (maxima[i].fEndX >= fSTARTXPeaks[j] - 1) &&
          (maxima[i].fStartY <= fENDYPeaks[j] + 1) && (maxima[i].fEndY >= fSTARTYPeaks[j] - 1)) {
        // merge
        merged = kTRUE;
        if (fWeight[j] > 0) {
          fXPeaks[fNPeaks] =
            (hist->GetPreciseBinCenterX(maxima[i].fX) + (fENDETAPeaks[j] - fSTARTETAPeaks[j] + 1) * fXPeaks[j]) /
            (fENDETAPeaks[j] - fSTARTETAPeaks[j] + 2);
          fYPeaks[fNPeaks] =
            (hist->GetPreciseBinCenterY(maxima[i].fY) + (fENDETAPeaks[j] - fSTARTETAPeaks[j] + 1) * fYPeaks[j]) /
            (fENDETAPeaks[j] - fSTARTETAPeaks[j] + 2);
...
void MaxFinder::FindPeak1(Int_t ywindow, Int_t xbinsides)
{
  ...
  if (!fCurrentHisto) {
    printf("MaxFinder::FindPeak1 : No input histogram\n");
    return;
  }
  if (fCurrentHisto->GetNEntries() == 0)
    return;
  ...
  for (Int_t i = 0; i < nbinsx; i++) {
    ...
    fNPeaks++;
    if (fNPeaks == fNMax) {
      cerr << "MaxFinder::FindPeak1 : Peak array out of range!!!" << endl;
      break;
    }
  }
  ...
  for (Int_t i = 0; i < fNPeaks; i++) {
    ...
    if (xbin - xbinsides < xmin || xbin + xbinsides > xmax)
      continue;
    top = butt = 0;
    ytop = 0, ybutt = 0;
    w = 0;
...
void MaxFinder::SortPeaks(struct AliHLTTPCAxisWindow** a, Int_t first, Int_t last)
{
...
Int_t MaxFinder::PeakCompare(struct AliHLTTPCAxisWindow* a, struct AliHLTTPCAxisWindow* b) const
{
...
void MaxFinder::FindPeak(Int_t t1, Double_t t2, Int_t t3)
{
  ...
  if (!fCurrentHisto) {
    printf("MaxFinder::FindPeak : No histogram!!\n");
    return;
  }
  Accumulator* hist = fCurrentHisto;
  if (hist->GetNEntries() == 0)
...
moveon:
  ...
  if (hist->GetBinCenterX(xl) > xpeak || hist->GetBinCenterX(xu) <= xpeak)
    printf("\nMaxFinder::FindPeak : Wrong xrange %f %f %f\n\n", hist->GetBinCenterX(xl), xpeak,
           hist->GetBinCenterX(xu));
...
Float_t MaxFinder::GetXPeakSize(Int_t i) const
{
  // Get X size of a peak
  if (i < 0 || i > fNMax) {
    STDCERR << "MaxFinder::GetXPeakSize : Invalid index " << i << STDENDL;
    return 0;
  }
...
Float_t MaxFinder::GetYPeakSize(Int_t i) const
{
  // Get Y size of a peak
  if (i < 0 || i > fNMax) {
    STDCERR << "MaxFinder::GetYPeak : Invalid index " << i << STDENDL;
    return 0;
  }
...
Bool_t MaxFinder::MergeRowPeaks(AliHLTTPCPre2DPeak* maxima1, AliHLTTPCPre2DPeak* maxima2, Float_t distance)
{
  ...
  Float_t psi1 = atan((x1 - y1) / (TransformerRow::GetBeta1() - TransformerRow::GetBeta2()));
  Float_t kappa1 = 2.0 * (x1 * cos(psi1) - TransformerRow::GetBeta1() * sin(psi1));
  track1.SetTrackParameters(kappa1, psi1, 1);
  Float_t firsthit1[3];
  if (!track1.GetCrossingPoint(firstrow, firsthit1))
    return kFALSE;
  Float_t lasthit1[3];
  if (!track1.GetCrossingPoint(lastrow, lasthit1))
    return kFALSE;

  HoughTrack track2;
  Float_t x2 = hist->GetPreciseBinCenterX(xtrack2);
  Float_t y2 = hist->GetPreciseBinCenterY(ytrack2);
  Float_t psi2 = atan((x2 - y2) / (TransformerRow::GetBeta1() - TransformerRow::GetBeta2()));
  Float_t kappa2 = 2.0 * (x2 * cos(psi2) - TransformerRow::GetBeta1() * sin(psi2));
  track2.SetTrackParameters(kappa2, psi2, 1);
  Float_t firsthit2[3];
  if (!track2.GetCrossingPoint(firstrow, firsthit2))
    return kFALSE;
  Float_t lasthit2[3];
  if (!track2.GetCrossingPoint(lastrow, lasthit2))
    return kFALSE;

  Float_t padpitchlow = Transform::GetPadPitchWidth(Transform::GetPatch(firstrow));
  Float_t padpitchup = Transform::GetPadPitchWidth(Transform::GetPatch(lastrow));
  // check the distance between tracks at the edges
  //  cout<<"Check "<<firsthit1[1]<<" "<<firsthit2[1]<<" "<<padpitchlow<<" "<<lasthit1[1]<<" "<<lasthit2[1]<<"
  //  "<<padpitchup<<" "<<xtrack1<<" "<<ytrack1<<" "<<xtrack2<<" "<<ytrack2<<endl;
