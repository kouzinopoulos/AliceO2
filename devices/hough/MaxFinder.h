...
#ifndef ALICEO2_HOUGH_MAXFINDER_H_
#define ALICEO2_HOUGH_MAXFINDER_H_
...
public:
  MaxFinder();
  MaxFinder(Char_t* histotype, Int_t nmax, Accumulator* hist = 0);
  virtual ~MaxFinder();
  void Reset();
  ...
  // Setters:
  void SetGradient(Float_t x, Float_t y)
  {
    fGradX = x;
    fGradY = y;
  }
  void SetThreshold(Int_t f) { fThreshold = f; }
  void SetHistogram(Accumulator* hist) { fCurrentHisto = hist; }
  void SetTrackLUTs(UChar_t* tracknrows, UChar_t* trackfirstrow, UChar_t* tracklastrow, UChar_t* nextrow)
  {
    fTrackNRows = tracknrows;
    fTrackFirstRow = trackfirstrow;
    fTrackLastRow = tracklastrow;
    fNextRow = nextrow;
  }
...
inline Float_t MaxFinder::GetXPeak(Int_t i) const
{
  if (i < 0 || i > fNMax) {
    STDCERR << "MaxFinder::GetXPeak : Invalid index " << i << STDENDL;
    return 0;
  }
  return fXPeaks[i];
}

inline Float_t MaxFinder::GetYPeak(Int_t i) const
{
  if (i < 0 || i > fNMax) {
...
inline Int_t MaxFinder::GetWeight(Int_t i) const
{
  if (i < 0 || i > fNMax) {
    STDCERR << "MaxFinder::GetWeight : Invalid index " << i << STDENDL;
    return 0;
  }
  return fWeight[i];
}

inline Int_t MaxFinder::GetStartEta(Int_t i) const
{
  if (i < 0 || i > fNMax) {
    STDCERR << "MaxFinder::GetStartEta : Invalid index " << i << STDENDL;
    return 0;
  }
  return fSTARTETAPeaks[i];
}

inline Int_t MaxFinder::GetEndEta(Int_t i) const
{
  if (i < 0 || i > fNMax) {
    STDCERR << "MaxFinder::GetStartEta : Invalid index " << i << STDENDL;
    return 0;
