// @(#) $Id: HoughTransformerRow.h 15995 2006-11-30 17:45:45Z hristov $
...
class HoughTransformerRow : public HoughBaseTransformer {

public:
  HoughTransformerRow();
  HoughTransformerRow(Int_t slice, Int_t patch, Int_t netasegments, Bool_t DoMC = false, Float_t zvertex = 0.0);
  virtual ~HoughTransformerRow();

  void CreateHistograms(Float_t ptmin, Float_t ptmax, Float_t pres, Int_t nybin, Float_t psi)
  {
    HoughBaseTransformer::CreateHistograms(ptmin, ptmax, pres, nybin, psi);
  }
  ...
  void TransformCircle(Int_t* row_range, Int_t every) { HoughBaseTransformer::TransformCircle(row_range, every); }
...
private:
  ...
  void SetTransformerArrays(HoughTransformerRow* tr);
...
typedef HoughTransformerRow AliL3HoughTransformerRow; // for backward compatibility
}
}
